$(document).ready(function onDocumentReady() {
    function adjustTooltipOffset(wrapper) {
        ...
        var documentWidth = document.body.clientWidth; // 문서 너비
        var wrapperLeft = $(wrapper).offset().left; // 툴팁래퍼 위치 (left)
        var wrapperWidth = parseFloat(window.getComputedStyle(wrapper).width); // 툴팁래퍼 너비
        var offsetPer = (wrapperLeft / documentWidth) * 100; // 문서 폭 대비 툴팁래퍼 위치
         ...
         * 60vw ~: 오른쪽에 위치
         */
        if (offsetPer < 40.0) { // 툴팁래퍼가 왼쪽에 위치한 경우
            // 툴팁을 오른쪽으로 전개
            $tooltip
                .removeClass("left middle")
                .css("max-width", (documentWidth - wrapperLeft) + "px");
        } else if (offsetPer < 60.0) { // 툴팁래퍼가 중앙에 위치한 경우
            // 툴팁을 중앙으로 전개
            $tooltip
                .removeClass("left")
                .addClass("middle")
                .css("max-width", (Math.min(wrapperLeft, (documentWidth - wrapperLeft)) * 2) + "px");
        } else { // 툴팁래퍼가 오른쪽에 위치한 경우
            // 툴팁을 왼쪽으로 전개
            $tooltip
                .removeClass("middle")
                .addClass("left")
